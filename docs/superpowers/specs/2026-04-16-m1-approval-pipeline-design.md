...
- `/pause`, `/resume`, `/quiet`, `/run`, `/sources`, `/providers` commands (M2/M3)
- `force_reply` timeout handling (30-min modify, 5-min reject)
...
| User sends modify text | poller | old draft `status: superseded` | — |
...
Inline keyboard `callback_data` format: `approve:{draft-id}`, `modify:{draft-id}`, `reject:{draft-id}`.

Constraint: callback_data max 64 bytes — draft IDs must stay under ~50 chars.
...
**Approve:** `answerCallbackQuery` "Approved!" → edit message to strikethrough + "✅ Approved" → update state.json → call archive → archive sends Template B.
...
**Modify:** `answerCallbackQuery` "Send your changes" → edit message to strikethrough + "✏️ Awaiting changes..." → set `status: modifying`. Poller watches for next text message from paired user → calls provider-router to regenerate → creates new draft with `parent_id` → supersedes old → sends new through approval.
...
| `/queue` | List all drafts in `pending/` with status. Empty → "No pending drafts." |
